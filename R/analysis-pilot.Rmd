...
```{r}
train_times = tibble(independent_edge_nh = 4, if2_nnl = 4, if2_hnh = 10,
                     if1_ln = 3, if2_nnh = 7, if1_nn = 2, if1_hn = 6) %>% 
...
train.time_total %>% 
  ...
  labs(y = "total training time in sec", x = "nb correct training trials")
...
```{r}
conditions = list(neutral="Which blocks do you think will fall?",
                  if_ant = "What happens if the 'antecedent' block falls?",
                  cons = "Will the 'consequent' block fall?")
...
**General thoughts** (for potential follow-up experiment):

  - Maybe show two pictures only after participants have read Ann's question? 
