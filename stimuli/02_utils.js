...
// converts array of all conditions into object with relations as keys mapping
// to array of corresponding conditions
sortConditions = function(conditions){
  let filtered = {};
  let iff = [];
  TEST_TYPES.forEach(function(rel){
    filtered[rel] = [];
  })
  conditions.forEach(function(arr){
    filtered[_.last(arr)].push(arr);
  });
  return filtered
}
